...
`nsEVDx` is an open-source Python package for fitting stationary and non-stationary Extreme Value Distributions (EVDs) to extreme value data. It supports non-stationary models in which the location, scale, and shape parameters can each depend on user-defined covariates. Overall, `nsEVDx` provides an easy-to-use, flexible, and extensible Python-native interface for frequentist and Bayesian inference of non-stationary EVD models. This enables researchers and practitioners to quantify changing risks associated with rare and extreme events.
...
In the Python ecosystem, `pyextremes` is a well-established package for extreme value analysis, supporting block maxima and peaks-over-threshold extraction, return-level estimation, `SciPy` based frequentist parameter estimation and Bayesian inference through the affine-invariant ensemble sampler, `emcee`[@emcee]. However, `pyextremes` is primarily focused on stationary extreme value workflows. In contrast, nsEVDx provides a unified framework for both stationary and non-stationary EVDs, allowing arbitrary covariates to modulate the location, scale, and shape parameters through a configurable model structure. nsEVDx also expands Bayesian inference by including including Random-Walk Metropolis, MALA, and Hamiltonian Monte Carlo that are not included in `pyextremes`.  

These differences motivate a dedicated Python tool that combines flexibility with an accessible interface for stationary and non-stationary EVD modeling. In particular, `nsEVDx` supports user-defined covariates, parameter constraints, custom priors, and MCMC algorithms such as MALA [@roberts_exponential_1996] and HMC [@michael_betancourt_conceptual_2017;neal2011mcmc] for fitting non-stationary GEV and GPD models. Compared with general-purpose probabilistic programming frameworks, `nsEVDx` provides predefined EVD likelihoods, parameterizations, and inference workflows, reducing the amount of model construction required for common extreme value analyses. 
...
A simulation-based case study was performed to compare mean absolute errors (MAE) in estimating EVD parameters across multiple packages, including `nsEVDx`. First, we generated $N=100$ stationary and $N=50$ non-stationary datasets, each consisting 500 extreme values sampled from using EVD parameter sets (hereafter referred to as the "true" parameter; Figure 3). These datasets were then used to fit EVD models, and MAE was computed by comparing estimated parameters with the corresponding true parameters.
